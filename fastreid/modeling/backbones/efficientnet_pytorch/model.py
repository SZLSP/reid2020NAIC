...
def build_efficientnet_backbone(cfg):
    ...
    if pretrain:
        pretrain_path = None if len(pretrain_path) == 0 else pretrain_path
        model = EfficientNet.from_pretrained(model_name, pretrain_path, num_classes=1, image_size=img_size)
    else:
        model = EfficientNet.from_name(model_name, num_classes=1, image_size=img_size)

    cfg.defrost()
    cfg.MODEL.HEADS.IN_FEAT = model._fc.in_features
    cfg.freeze()
