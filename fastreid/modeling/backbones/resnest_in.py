...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, bn_norm, num_splits, with_ibn=False, stride=1, downsample=None,
                 ...
                 dropblock_prob=0.0, last_gamma=False, IN=True):
        ...
        self.IN = IN
        if IN:
            self.in_layer = get_norm(IN2d, planes * 4, num_splits)
    ...
    def __init__(self, last_stride, bn_norm, num_splits, with_ibn, with_nl, block, layers, non_layers, radix=1,
        ...
        if deep_stem:
            self.conv1 = nn.Sequential(
                ...
                get_norm(IN2d, stem_width, num_splits),
        ...
        self.bn1 = get_norm(IN2d, self.inplanes, num_splits)
        self.relu = nn.ReLU(inplace=True)
...
def build_resnest_in_backbone(cfg):
    """
